...
void test_board_remove_small_block(void) {
    ...
    board_remove_small_block(b, 2, 2);
...
void test_board_remove_horizontal_i(void) {
    ...
    board_remove_horizontal_i(b, 1, 2);
...
void test_board_remove_piece_big_square(void) {
    ...
    board_remove_piece(b, PIECE_BIG_SQUARE, 1, 1);
...
void test_board_remove_piece_small_block(void) {
    ...
    board_remove_piece(b, PIECE_SMALL_BLOCK, 0, 2);
...
void test_board_remove_piece_vertical_i(void) {
    ...
    board_remove_piece(b, PIECE_VERTICAL_I, 1, 1);
...
void test_board_remove_piece_horizontal_i(void) {
    ...
    board_remove_piece(b, PIECE_HORIZONTAL_I, 3, 4);

    TEST_ASSERT_EQUAL(1, b->num_horizontal);
    destroy_board(b);
}

void test_board_get_piece_position_big_square(void) {
    board_t *b = build_board();
    b->big_piece = set_big_square_position(1, 1);

    uint_fast16_t px, py;
    board_get_piece_position(b, PIECE_BIG_SQUARE, 1, 1, &px, &py);
    TEST_ASSERT_EQUAL(1, px);
    TEST_ASSERT_EQUAL(1, py);

    board_get_piece_position(b, PIECE_BIG_SQUARE, 2, 2, &px, &py);
    TEST_ASSERT_EQUAL(1, px);
    TEST_ASSERT_EQUAL(1, py);
...
void test_board_get_piece_position_small_block(void) {
    board_t *b = build_board();
    b->small_blocks = set_small_block_position(b->small_blocks, 0, 3, 0);
    b->small_blocks = set_small_block_position(b->small_blocks, 1, 0, 2);

    uint_fast16_t px, py;
    board_get_piece_position(b, PIECE_SMALL_BLOCK, 0, 2, &px, &py);
...
void test_board_get_piece_position_vertical_i(void) {
    board_t *b = build_board();
    b->num_vertical = 2;
    b->vertical_blocks = set_vertical_i_position(b->vertical_blocks, 0, 1, 0);
    b->vertical_blocks = set_vertical_i_position(b->vertical_blocks, 1, 3, 2);
    ...
    uint_fast16_t px, py;
    board_get_piece_position(b, PIECE_VERTICAL_I, 1, 0, &px, &py);
    TEST_ASSERT_EQUAL(1, px);
    TEST_ASSERT_EQUAL(0, py);

    board_get_piece_position(b, PIECE_VERTICAL_I, 1, 1, &px, &py);
    TEST_ASSERT_EQUAL(1, px);
    TEST_ASSERT_EQUAL(0, py);
...
void test_board_get_piece_position_horizontal_i(void) {
    board_t *b = build_board();
    b->num_horizontal = 1;
    b->horizontal_blocks = set_horizontal_i_position(b->horizontal_blocks, 0, 1, 2);

    uint_fast16_t px, py;
    board_get_piece_position(b, PIECE_HORIZONTAL_I, 1, 2, &px, &py);
    TEST_ASSERT_EQUAL(1, px);
    TEST_ASSERT_EQUAL(2, py);

    board_get_piece_position(b, PIECE_HORIZONTAL_I, 2, 2, &px, &py);
    TEST_ASSERT_EQUAL(1, px);
    TEST_ASSERT_EQUAL(2, py);
...
int main(void) {
    ...
    RUN_TEST(test_board_get_piece_position_big_square);
